...
def ai_agent_router():
    ...
    if vid:
            ...
            msg =  f"playing {q} "
    elif  any (k in  cmd for k in ["gmail", "mail", "email"]): 
     to, body = "",""
     cleaned_cmd = re.sub(
          r'^(please\s+)?(open\s+)?(gmail|mail|email)\s*',
            '',
            cmd
).strip()
...
recip_parts = parts[0].strip()

recip_parts = re.sub(r'^(update\s+to|to|send\s+to|and\s+update\s+to)\s*', '', recip_parts).strip()
if len(parts) > 1:
     body_parts = parts[-1].strip()
if recip_parts:
     c = recip_parts.replace("at ", "@").replace("dot", ".").replace(" ", "")    
